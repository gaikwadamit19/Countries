...
class CoreDataManager {
    ...
    func insertCountry(flagUrl: String?, flagImage: Data?, name: String?, capital: String?, callingCodes: [String]?, region: String?, subRegion: String?, timeZones: [String]?, currencies: [Currencies]?, languages: [Languages]?) -> Country_? {
        ...
        for currency in currencies ?? [] {
            ...
            let currenciesDescrpt = NSManagedObject(entity: currenciesEntityDescriptor,
                                      insertInto: managedContext)
        ...
        for language in languages ?? [] {
            ...
            let languageDescrpt = NSManagedObject(entity: languageEntityDescriptor,
                                                insertInto: managedContext)
        ...
        //country.setValue(currencies, forKeyPath: CoreDataStrings.properties.currencies)
        //country.setValue(languages, forKeyPath: CoreDataStrings.properties.languages)
        
//        var currencyValue: String = ""
//        currencyValue = currencies?.reduce(String()) { (_, currency) in
//            return currencyValue.appendingFormat("\(CurrencyStrings.name): \(currency.name ?? "")\n\(CurrencyStrings.code): \(currency.code ?? "")\n\(CurrencyStrings.symbol): \(currency.symbol ?? "")")
//        } ?? DefaultValues.label
//        country.setValue(currencyValue, forKeyPath: CoreDataStrings.properties.currencies)
//
//        var languageString: String = ""
//        languageString = languages?.reduce(String()) { (_, language) in
//            return languageString.appendingFormat("\(LanguageStrings.name): \(language.name ?? "")\n\(LanguageStrings.nativeName):\(language.nativeName ?? "")\n\(LanguageStrings.iso1): \(language.iso639_1 ?? "")\n\(LanguageStrings.iso2): \(language.iso639_2 ?? "")")
//            } ?? DefaultValues.label
//        country.setValue(languageString, forKeyPath: CoreDataStrings.properties.languages)
